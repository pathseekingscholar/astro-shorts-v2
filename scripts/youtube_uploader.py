...
Uploads the newest rendered script/video pair that has not already been uploaded.
This avoids mismatching a fresh title with an old MP4 that happens to be sitting in videos_output.
"""
...
def get_upload_candidates(scripts_dir="scripts_output"):
    ...
    for filename in os.listdir(scripts_dir):
        ...
        is_rendered = data.get("status") == "rendered" or bool(data.get("rendered"))
        if not is_rendered:
            continue
...
def main():
    print("=" * 60)
    print("ASTRO SHORTS V2 - YouTube Uploader")
    print("=" * 60)
    print()
